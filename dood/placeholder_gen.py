...
- SIZE: default 64px. For tiny 32px sprites:
  ...
  (--size sets node 83 PrimitiveInt "RESOLUTION HERE"; latent/crop/output
    derive from it via node 85 (a*8), so 16, 32, 64 and 96 are supported.
    16 is for tiny abstract bits only (blood drops, sparks) — never characters.
    96 needs ~768px latent/diffusion + more VRAM/time.)
- BIG SPRITE (pot etc): python placeholder_gen.py --prompt "a cauldron" --out ./pot.png --size 96
...
- NEGATIVE: --negative "face, eyes, character" injects into node 99
  (PrimitiveString "OPTIONAL NEGATIVE PROMPT" -> node 100 suffix -> node 10),
  so the fixed negative suffix survives. Workflows exported without the
  node 99 slot: --negative overrides node 10 text directly.
...
Workflow nodes that matter: 75 prompt -> 74 style suffix -> 7 positive;
99 negative subject -> 100 negative suffix -> 10 negative;
...
def inject_prompt_smart(wf, prompt, negative):
    ...
    feeding CLIPTextEncode, so the style suffixes survive.
    Positive chain: node 75 "PROMPT/SUBJECT HERE" -> 74 -> 7.
    Negative chain (new workflows): node 99 "OPTIONAL NEGATIVE PROMPT" -> 100 -> 10.
    Legacy workflows without the node 99 slot: --negative overrides node 10 text directly."""
    def upstream_prim(cid):
        sa = wf[cid]["inputs"].get("string_a")
        if isinstance(sa, list) and str(sa[0]) in wf and \
           wf[str(sa[0])].get("class_type") == "PrimitiveString":
            return str(sa[0])
        return None
    # 1. StringConcatenate nodes whose output feeds a CLIPTextEncode
    concats = [nid for nid, n in wf.items()
               if n.get("class_type") == "StringConcatenate"
               and any(n2.get("class_type") == "CLIPTextEncode"
                       and isinstance(n2["inputs"].get("text"), list)
                       and str(n2["inputs"]["text"][0]) == nid
                       for n2 in wf.values())]
    pos_id = neg_id = None
    for cid in concats:
        pid = upstream_prim(cid)
        title = str(wf[pid].get("_meta", {}).get("title", "")).upper() if pid else ""
        if "NEGATIVE" in title:
            neg_id = cid
        elif "PROMPT" in title and pos_id is None:
            pos_id = cid
    if pos_id is None and concats:
        pos_id = concats[0]  # legacy single-chain guess
    if pos_id is not None and upstream_prim(pos_id) is not None:
        wf[upstream_prim(pos_id)]["inputs"]["value"] = prompt
        where = f"PrimitiveString {upstream_prim(pos_id)} (via StringConcatenate {pos_id})"
    else:
    ...
    if negative:
        if neg_id is not None and upstream_prim(neg_id) is not None:
            wf[upstream_prim(neg_id)]["inputs"]["value"] = negative
            where += f" + negative -> PrimitiveString {upstream_prim(neg_id)} (via StringConcatenate {neg_id})"
        else:
            # legacy workflow: node 10 holds the negative as plain text
            for nid, n in wf.items():
                if n.get("class_type") == "CLIPTextEncode" and isinstance(n["inputs"].get("text"), str):
                    n["inputs"]["text"] = negative
                    where += f" + negative -> CLIPTextEncode {nid} (direct override)"
                    break
    return where
...
def main():
    ...
    ap.add_argument("--negative", default=None, help="Negative prompt subject, e.g. 'face, eyes'. Appended in front of the workflow's fixed negative suffix (node 99 slot, else node 10 override)")
    ...
    ap.add_argument("--size", type=int, choices=[16, 32, 64, 96], default=64, help="Sprite resolution in px (sets node 83 RESOLUTION HERE; default: 64, 96 for big sprites like the pot; 16 for tiny abstract bits only)")
    ...
    # Resolution: node 83 PrimitiveInt "RESOLUTION HERE" (16|32|64|96).
    # Node 85 derives latent/crop/output px (a*8), so this one value drives all sizes.
    # 96 -> 768px diffusion: slower + more VRAM. If OOM, lower steps via --set "3.inputs.steps=12".
